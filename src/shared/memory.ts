...
export function coerceKeywords(value: string[] | string): string[] {
  ...
  return Array.from(new Set(list.map(normalizeToken).filter(Boolean))).slice(0, 12);
}

export function validateDecisionInput(input: DecisionInput): string[] {
  const errors: string[] = [];
  if (!input.thingId || input.thingId.length > 80) errors.push("thingId is required.");
  if (!["post", "comment"].includes(input.thingType)) errors.push("thingType is invalid.");
  if (!input.ruleTag || input.ruleTag.length > 40) errors.push("ruleTag is required.");
  if (!OUTCOMES.includes(input.outcome)) errors.push("outcome is invalid.");
  if (!input.summary || input.summary.length > 320) errors.push("summary must be 1-320 characters.");
...
export function createDecisionRecord(input: DecisionInput, now = new Date()): DecisionRecord {
...
export function matchPrecedents(
  ...
  return decisions
    .map((decision) => {
      ...
      const keywordHits = decision.keywords.filter((keyword) => itemTokens.has(keyword));
    ...
    .sort((a, b) => b.score - a.score || b.decision.createdAt.localeCompare(a.decision.createdAt))
    .slice(0, limit);
}
...
export function cleanupExpired(
  ...
  decisions.forEach((decision) => {
    if (new Date(decision.expiresAt).getTime() <= now.getTime()) expired.push(decision);
